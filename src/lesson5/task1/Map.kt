...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((key, value) in mapB) {
        result.merge(key, value) { valueA, valueB -> if (valueA == valueB) valueA else "$valueA, $valueB" }
    }
    return result
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((first, second) in stockPrices) {
        result.getOrPut(first) { mutableListOf() }.add(second)
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    var minCount = Double.POSITIVE_INFINITY
    var finalName: String? = null
    for ((name, info) in stuff) {
        if (info.first == kind && info.second < minCount) {
            minCount = info.second
            finalName = name
        }
    }
    return finalName
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val charsRe = chars.map { it.toLowerCase() }.toSet()
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in 0 until list.size) {
        if (list[i] in res) return res[list[i]]!! to i
        else res[number - list[i]] = i
    }
    return Pair(-1, -1)
}
